...
──────────────────────────────────────────────────────────────────────
  USAGE        يقال / فيقال / يقولون / تقول   (2,303+ occurrences)
  AUTHORITY    قال X : / قالوا :               (3,451+ occurrences)
  BRANCH       ومن الباب / ومن هذا الباب       (414 occurrences)
  INTRO        من ذلك / فمن ذلك                (502 occurrences)
  SECOND_ORIG  والأصل الآخر / والثانى          (188 occurrences)
  ...
  CONDITIONAL  إذا (defines a usage condition) (2,039 occurrences)
  PLURAL       والجمع / وجمعه                  (185 occurrences)
  CONTINUATION (default — unmarked prose)

OCR normalisation applied before matching:
  وبقال → ويقال    (62 OCR artifacts corrected)
  يقولون → يقال    (926 OCR artifacts normalised)
"""
...
#   Each entry: (segment_type, compiled_regex, priority)
...
LEXICAL_MARKERS: dict[str, dict[str, Any]] = {
    ...
    "يقولون": {
        "type": "USAGE",
        "pattern": r"(?:و)?يقولون\b|(?:و)?تقول\b|(?:و)?يقولون\b",
        "freq": 926,
        "note": "alternate usage introduction; OCR sometimes renders يقال as يقولون",
    },
    "قيل": {
        "type": "USAGE",
        "pattern": r"\bقيل\b",
    ...
    "قال": {
        ...
        "freq": 3453,
        "note": "attributes text to a named authority; top: ابن دريد, أبو بكر, أبو عبيد",
    },
    ...
    "والأصل الآخر": {
        ...
        "freq": 265,
        "note": "signals DUAL-origin entries; marks the split between the two origins",
    },
...
# ── Compiled split-pattern (used by segment_body) ───────────────────────────

def _build_split_regex() -> re.Pattern:
    """
    Build a single alternation regex from all marker patterns,
    ordered so longer/more-specific patterns come first.
    The regex captures the matched marker so split() keeps it.
    """
    # Ordered by priority: structural > usage > authority > etymological
    priority_order = [
        "ومما شذّ",
        "والأصل الآخر",
        "ومن الباب",
...
def _classify_marker(marker_text: str) -> str:
    """Return the segment type for a matched marker string."""
...
def _best_text(line_obj: dict[str, Any]) -> str:
    """
    Return the best available text for a line:
...
class MaqayisBodyLoader:
    ...
    def get_body_text(
        ...
        *,
        ...
        OCR normalisation applied (وبقال → ويقال, etc.).
        """
    ...
    def segment_body(
        ...
        *,
        ...
        Each span is labelled with the discourse marker that opened it:
          USAGE        يقال / فيقال / يقولون
          ...
          ETYMOLOGY    أصله / لأنّه / كأنّه / سُمِّي
          HADITH       وفى الحديث
          PLURAL       والجمع / وجمعه
          CONTINUATION (opening prose before first marker)

        Parameters
        ──────────
        entry            : dict from root_entries JSONL
        include_footnotes: include footnote lines in body
        min_text_len     : skip segments whose text is shorter than this

        Returns
        ───────
        list[BodySegment] — ordered list of typed spans
        """
        body = self.get_body_text(entry, include_footnotes=include_footnotes)
        if not body:
            return []

        # Split on markers, keeping markers in result
        ...
        for part in parts:
            ...
            # Check if this part is itself a marker
            if _SPLIT_RE.fullmatch(part.strip()):
                ...
                # Reset after placing a segment (next segment gets fresh marker)
                current_marker = ""
                current_type   = "CONTINUATION"
...
def get_default_loader(lines_jsonl: Optional[pathlib.Path] = None) -> MaqayisBodyLoader:
    ...
    Subsequent calls return the cached instance (path argument ignored).
    """
